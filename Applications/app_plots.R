...
#---------------------------Figure A.24,A.26,A.28----------------
truncation_vec <- c(0.95)

pixel_x_vec <- c(64,32,256)
pixel_y_vec <- c(64,32,256)
for (pixel_index in 1:length(pixel_x_vec)) {
  pixel_x <- pixel_x_vec[pixel_index]
  pixel_y <- pixel_y_vec[pixel_index]

  for (truncation in 0.95) {
    for (al in 1) {
      ...
      for (oo in c("SAF","IED")) {
        for (mm in 1:10) {
          scale <- 81/(128/pixel_x*128/pixel_y)
          load(paste0(save_path, "res_Type1_M", mm,"_",oo,"_trun_aid_bi_",truncation*100,".dat"))
          m <- res$est_beta[2]*scale
          V <- diag(res$V_beta)[2]*scale^2
...
for (truncation in truncation_vec) {
  for (al in 1:length(aid_lag)) {
    save_path <- paste0(outpath,"adaptive/",aid_lag_name[al],"/")
