...
# (1) generate data (for spatio-temporal effect modifier setting),
...
# setwd('/orange/gpapadogeorgou/Lingxiao/')
library(spatstat)
...
wh_sims <- 15       # name of the simulation
...
type_em <- 3       # type of the effect modifier: 1 for spatial and 3 for spatio-temporal 
...
# B <- list(B1 = window, B2 = baghdad_window, B3 = mosul_window)
B <- window
...
# na.im <- as.im(na.matrix,W = window)
...
# ---------- STEP 3: Calculating the estimand (CATE, Variance bound, Hypothesis test)----------- 
...
if (perform_estimand) {
  ...
  counter <- GenPotOut(intervention = intervention,
                       ...
                       reps = reps, short = FALSE,separate_prev_trt = (interaction!=1),interaction = interaction,linear = linear)
...
if (perform_estimation) {
  ...
  #save(estimates, file = paste0(out_path, 'estimates_', index, '.dat'))
