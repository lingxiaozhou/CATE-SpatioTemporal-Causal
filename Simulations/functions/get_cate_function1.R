...
#' 
#' Function that calculates the conditional average treatment effects: Compute the estimates for potentical outcomes under each intervention, using different variance bound estimator.
#' 
...
#' 
get_cate1 <- function(surface1, surface2,
                     E.basis,t.basis,point.basis,used_time=NULL, 
                     C=1,Hajek = TRUE,weights = NULL, separate_model=TRUE,confidence_region=TRUE,perform_pred = FALSE,M=1,weights_M1 = NULL){
  ...
  est <- na.omit((c(surface2)-c(surface1)))/C
  ...
  if(separate_model == FALSE){
    ...
    cat("get the asymptotic vairance\n")
    if(Hajek==FALSE){
    ...
    if(Hajek==TRUE){
      ...
      # Vt_haj <- array(NA,c(p,p,time_points))
    }else{
      beta_arr <- array(NA,c(time_points,p))
      Vt <- array(NA,c(p,p,time_points))
    }
    ...
    for (tt in 1:time_points) {
      ...
      tryCatch({
          ...
          if(Hajek==TRUE){
            ...
            # Vt_haj_tmp <- beta_arr[tt,(p+1):(2*p)]/mean(weights[2,])-beta_arr[tt,1:p]/mean(weights[1,])
            # Vt_haj[,,tt] <- Vt_haj_tmp%*%t(Vt_haj_tmp)
            if(perform_pred){
    ...
    if(Hajek==TRUE){
      ...
      estV <- estJ%*%Vt%*%t(estJ) # variance for beta_hat
    ...
    # if(Hajek==TRUE){
    #   Vt_haj <- points.basis%*%Vt_haj%*%t(points.basis)/time_points
    # }
    ...
    return(list(points = points, est = est, V.points = V.points,meaneffect = meaneffect,valid_time_points = valid_time_points, beta = beta, V.points.est = V.points.est,test.c = test.c,reject = reject,estV = estV/time_points,p.value = p.value,pred=pred,beta_arr = beta_arr,estJ=estJ,V.points.adhoc=V.points.adhoc))
  }
